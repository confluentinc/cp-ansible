...
Default:  "{{mds_http_protocol}}://{{ groups['kafka_broker'] | default(['localhost']) | confluent.platform.resolve_hostnames(hostvars) | map('confluent.platform.format_hostname') | join(':' + mds_port|string + ',' + mds_http_protocol + '://') }}:{{mds_port}}"
