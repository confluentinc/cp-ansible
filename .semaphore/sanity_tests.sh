...
# Exit with URI auth check result if it failed
if [ $URI_AUTH_EXIT_CODE -ne 0 ]; then
    echo "❌ Sanity tests failed due to URI authorization issues."
    exit $URI_AUTH_EXIT_CODE
fi
