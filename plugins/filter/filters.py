...
class FilterModule(object):
    ...
    def listener_properties(self, listeners_dict, default_ssl_enabled,
                            ...
                            scram256_password, oauth_cert_path, oauth_enabled, oauth_jwks_uri, oauth_expected_audience,
        ...
        for listener in listeners_dict:
            ...
            if self.normalize_sasl_protocol(listeners_dict[listener].get('sasl_protocol', default_sasl_protocol)) == 'OAUTHBEARER' and not oauth_enabled:
                final_dict['listener.name.' + listener_name + '.oauthbearer.sasl.jaas.config'] =\
                    'org.apache.kafka.common.security.oauthbearer.OAuthBearerLoginModule required publicKeyPath=\"' + oauth_cert_path + '\";'
            ...
            if self.normalize_sasl_protocol(listeners_dict[listener].get('sasl_protocol', default_sasl_protocol)) == 'OAUTHBEARER' and \
                ...
                if rbac_enabled and idp_self_signed:
                    final_dict['listener.name.' + listener_name + '.oauthbearer.sasl.jaas.config'] =\
                        ...
                        oauth_cert_path + '\" ssl.truststore.location=\"' + kafka_broker_truststore_path + \
                ...
                if (rbac_enabled and (not idp_self_signed)):
                    final_dict['listener.name.' + listener_name + '.oauthbearer.sasl.jaas.config'] =\
                        'org.apache.kafka.common.security.oauthbearer.OAuthBearerLoginModule required publicKeyPath=\"' + \
                        oauth_cert_path + '\" unsecuredLoginStringClaim_sub="thePrincipalName";'
    ...
    def client_properties(self, listener_dict, default_ssl_enabled, bouncy_castle_keystore, default_ssl_mutual_auth_enabled, default_sasl_protocol,
                          ...
                          omit_oauth_configs, oauth_username, oauth_password, mds_bootstrap_server_urls, oauth_enabled, oauth_client_id,
                          oauth_client_password, oauth_groups_scope, oauth_token_uri, idp_self_signed):
        ...
        if not omit_oauth_configs:
            ...
            if self.normalize_sasl_protocol(listener_dict.get('sasl_protocol', default_sasl_protocol)) == 'OAUTHBEARER' and oauth_enabled:
                ...
                if oauth_groups_scope == 'none' and idp_self_signed:
                    final_dict[config_prefix + 'sasl.jaas.config'] = 'org.apache.kafka.common.security.oauthbearer.OAuthBearerLoginModule required ' + \
                        'clientId=\"' + oauth_client_id + '\" clientSecret=\"' + str(oauth_client_password) + '\" ssl.truststore.location=\"' + \
